...
$SQLOrder = "SELECT * FROM `orders`, `user` WHERE `orders`.`Use_ID` = `user`.`Use_ID` AND `orders`.`Ord_Status` = '1' LIMIT " . $item_per_page . " OFFSET " . $offset;
$Order = $con->query($SQLOrder);

$total_records = mysqli_query($con, "SELECT * FROM `orders`, `user` WHERE `orders`.`Use_ID` = `user`.`Use_ID` AND `orders`.`Ord_Status` = '1'");
...
?>
<div class="order1">
    <h1><i class="fa fa-cart-plus"></i> Packaging</h1>
    <br>
