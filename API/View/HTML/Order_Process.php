<?php
    ...
    switch($action)
    {
        ...
        case 'order3':
        {
            if(isset($_POST['Ord_ID']) && !empty($_POST['Ord_ID']))
            {
                ...
                $ODID = $_POST['Ord_ID'];
                $SQLUpdate = "UPDATE `orders` SET `Ord_Status` = '3', `Ord_Date_Success` = '$current_date' WHERE `orders`.`Ord_ID` = '$ODID'";                   
                $Confirm = $con->query($SQLUpdate);
                if($Confirm)
                {
                    echo "Comfirm Success!";
    ...
    if($action == 'search')
    {
        ...
        {
            $Search = $_POST['Search'];
            $SQLOrder = "SELECT * FROM `orders`, `user` WHERE `orders`.`Use_ID` = `user`.`Use_ID` AND `orders`.`Ord_Status` = '0' AND `user`.`Use_Name` LIKE '%$Search%'";
...
?>
                <tr>
                    <td><?= $row['Ord_ID'] ?></td>
                    <td><?= $row['Use_Name'] ?></td>
                    <td><a href="Order_Detail.php?ODID=<?= $row['Ord_ID'] ?>&CUSID=<?= $row['Use_ID'] ?>" target="_blank" style="color: #06ba2a;">View Details</a></td>
