        ...
        {          
            ...
            if($From !== $To)
            {
                ...
                else
                {
                    $NumID = 0;
                    $Revenue = 0;
                    $SQLOrder = "SELECT * FROM `orders`, `user` WHERE `orders`.`Use_ID` = `user`.`Use_ID` AND `Ord_Status` = '3' AND `Ord_Date_Success` BETWEEN DATE('$From') AND DATE_ADD('$To', INTERVAL 1 DAY)";
